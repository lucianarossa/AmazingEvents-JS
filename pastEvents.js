...
//CREO EL ARRAY DE EVENTOS PAST PARA TOMAR COMO PARAMETROS DIRECTAMENTE LOS VALORES
...
async function getData() {
    ...
    let cardDate = objetoPrincipal.currentDate

    //-----------------------------------------------------------------------------------------------------------


    var arrayPast = eventos.filter(e => cardDate > e.date);
    ...
    let todasLasCategorias = arrayPast.map(evento => evento.category);
    ...
    function eventosFiltrados() {
        ...
        if (checksSelected.length > 0 && searchText !== "") {
            checksSelected.map(categoria => datos.push(...arrayPast.filter(evento => evento.name.toLowerCase().includes(searchText.trim().toLowerCase()) && evento.category == categoria)));
        } else if (checksSelected.length > 0 && searchText == "") {
            checksSelected.map(categoria => datos.push(...arrayPast.filter(evento => evento.category == categoria)));
        } else if (checksSelected.length == 0 && searchText !== "") {
            datos.push(...arrayPast.filter(evento => evento.name.toLowerCase().includes(searchText.trim().toLowerCase()) ||
                evento.category.toLowerCase().includes(searchText.trim().toLowerCase())));
        } else {
            datos.push(...arrayPast);
        }
        cardDatos(datos);
    }
    eventosFiltrados();

    //CREO LA FUNCION CARD DATOS QUE IMPRIMIRA LOS EVENTOS (LE PONGO COMO PARAMETRO EL ARRAY DE EVENTOS), LE PONGO UNA CONDICION QUE SI EL ARRAY TIENE DATOS, LO RECORRA CON FOREACH, A SU VEZ SI LA FECHA DEL EVENTO ES MENOR A LA FECHA BASE, LE IMPRIMA EN CARDCONTAINER TODOS LOS OBJETOS DEL ARRAY SEGUN SU INDICE, EN CAMBIO SI EL ARRAY ESTA VACIO ARROJE UN MENSAJE!
