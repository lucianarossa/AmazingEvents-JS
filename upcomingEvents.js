...
//CREO EL ARRAY DE EVENTOS FUT PARA TOMAR COMO PARAMETROS DIRECTAMENTE LOS VALORES
...
async function getData() {
    ...
    var arrayFut = eventos.filter(e => cardDate < e.date);
    ...
    let todasLasCategorias = arrayFut.map(evento => evento.category);
    ...
    function eventosFiltrados() {
        ...
        if (checksSelected.length > 0 && searchText !== "") {
            checksSelected.map(categoria => datos.push(...arrayFut.filter(evento => evento.name.toLowerCase().includes(searchText.trim().toLowerCase()) && evento.category == categoria)));
        } else if (checksSelected.length > 0 && searchText == "") {
            checksSelected.map(categoria => datos.push(...arrayFut.filter(evento => evento.category == categoria)));
        } else if (checksSelected.length == 0 && searchText !== "") {
            datos.push(...arrayFut.filter(evento => evento.name.toLowerCase().includes(searchText.trim().toLowerCase()) ||
    ...
    //CREO LA FUNCION CARD DATOS QUE IMPRIMIRA LOS EVENTOS (LE PONGO COMO PARAMETRO EL ARRAY DE EVENTOS), LE PONGO UNA CONDICION QUE SI EL ARRAY TIENE DATOS, LO RECORRA CON FOREACH, A SU VEZ SI LA FECHA DEL EVENTO ES MAYOR A LA FECHA BASE, LE IMPRIMA EN CARDCONTAINER TODOS LOS OBJETOS DEL ARRAY SEGUN SU INDICE, EN CAMBIO SI EL ARRAY ESTA VACIO ARROJE UN MENSAJE!
